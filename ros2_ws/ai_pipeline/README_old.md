...
The existing ROS package src/ailinker is untouched and can still be used later.  
**1) Prerequisites**  
1. Build ATRBridge core gateway binary:  
cd /home/shaon/mini-turtlebot/atrbridge_core  
 cmake -S . -B build  
 cmake --build build -j  
   
1. Install Python deps (no ROS needed):  
...
**6) Robust Metrics**  
Two CSV metric files are generated:  
...
Metrics files are append-only by default. Clear them before a new experiment batch.  
**Captured metrics**  
...
 ./start_rosfree_stack.sh --robot-ip 192.168.0.78 --robot-id tb_01  
   
1. Run one controlled scenario (example):  
cd /home/shaon/mini-turtlebot/ros2_ws/ai_pipeline  
 source .venv/bin/activate  
 export GEMINI_API_KEY="YOUR_GEMINI_API_KEY"  
 ./run_task.sh \  
...
   --llm-max-retries 2  
   
1. Repeat N runs per scenario (recommended N>=5), changing --task-id each run.  
2. Snapshot/copy CSV logs after each scenario batch:  
...
rm -f /tmp/minitb_ai_pipeline/ailinker_metrics.csv /tmp/minitb_ai_pipeline/behavior_metrics.csv  
   
1. Run each scenario N times (N>=5), fixed duration.  
2. Keep Wi-Fi/AP conditions fixed per batch.  
3. Use analyze_metrics.py for summary stats.  
4. Report p50/p95/p99 for:  
- gw_req.rtt_us[cmd_vel]  
- cmd_exec.latency_us  
- llm_call.latency_us  
- tool_call.latency_us[send_cmd_vel]  
5. Include failure rates:  
- cmd_exec.ok_rate  
